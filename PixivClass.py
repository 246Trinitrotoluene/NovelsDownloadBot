...
from configuration import novel_path, illust_path, addTranslatedTags, testMode
...
class PixivBase(PixivABC):  # 共用方法
	...
	def formatTags(tags: set) -> str:
		tags = list(tags)
		if ("R18" in tags or "R18G" in tags) and "SFW" in tags:
			tags.remove("SFW")
...
class PixivNovels(PixivBase):
	...
	def checkJson(self):
		if not self.json:
			raise RuntimeError("网络状态不佳，请联系管理员")
		...
		try:
			...
			# {'error': {'user_message': '', 'message': 'Error occurred at the OAuth process. Please check your Access Token to fix this. Error Message: invalid_grant'}
				...
				raise RuntimeError("Tokens 已失效，请联系管理员")
...
class PixivIllusts(PixivBase):
	...
	def getInfo(self):
		...
		self.tags = self.getTags(illust.tags)
		if illust.x_restrict == 0:
			self.tags.add("SFW")
		elif illust.x_restrict == 1:
			self.tags.add("R18")
		elif illust.x_restrict == 2:
			self.tags.add("R18G")
	...
	def __str__(self):
		self.info = f"{self.title}\nBy {self.author_name}\n" \
			f"阅读：{self.views}；收藏：{self.bookmarks}；评论：{self.comments}\n" \
			f"标签：{self.formatTags(self.tags)}\n{self.illust_url}\n"
		return self.info
	...
	def save(self, lang2=""):
		self.folder = os.path.join(illust_path, self.title)
		removeFile(self.folder)
		os.makedirs(self.folder)
		# print(f"{self.folder=}")
		self.files = []  #清空下载图片路径
		for i in range(0, len(self.original_urls)):
			tokenPool.getAPI().download(self.original_urls[i], path=self.folder, name=f"{self.title}{i+1}.jpg")
...
class PixivObject(object):
	...
	def saveAsZip(self, lang2=""):
		...
		if isinstance(self.obj, PixivSeries):
			...
			path = self.saveAuthor(lang2=lang2)
...
def test():  # tokenPoolInit() 不开线程才可用
	...
	# d3 = PixivObject("https://www.pixiv.net/artworks/103646563").save()
